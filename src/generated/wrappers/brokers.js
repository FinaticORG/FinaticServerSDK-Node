...
class BrokersWrapper {
    ...
    async getBrokers() {
        ...
        if (cache && this.sdkConfig?.cacheEnabled) {
            const cacheKey = (0, cache_1.generateCacheKey)('GET', '/api/v1/brokers/', {}, this.sdkConfig);
    ...
    async connectBroker(body) {
        ...
        const cache = (0, cache_1.getCache)(this.sdkConfig);
        if (cache && this.sdkConfig?.cacheEnabled) {
            const cacheKey = (0, cache_1.generateCacheKey)('POST', '/api/v1/brokers/connect', { body }, this.sdkConfig);
            const cached = cache.get(cacheKey);
            if (cached) {
                this.logger.debug('Cache hit', { request_id: requestId, cache_key: cacheKey });
                return cached;
    ...
    async listBrokerConnections() {
        ...
        const cache = (0, cache_1.getCache)(this.sdkConfig);
        if (cache && this.sdkConfig?.cacheEnabled) {
            const cacheKey = (0, cache_1.generateCacheKey)('GET', '/api/v1/brokers/connections', {}, this.sdkConfig);
            const cached = cache.get(cacheKey);
            if (cached) {
                this.logger.debug('Cache hit', { request_id: requestId, cache_key: cacheKey });
                return cached;
    ...
    async updateConnection(connectionId, body) {
        ...
        const cache = (0, cache_1.getCache)(this.sdkConfig);
        if (cache && this.sdkConfig?.cacheEnabled) {
            const cacheKey = (0, cache_1.generateCacheKey)('PUT', '/api/v1/brokers/connections/{connection_id}', { connectionId, body }, this.sdkConfig);
            const cached = cache.get(cacheKey);
            if (cached) {
                this.logger.debug('Cache hit', { request_id: requestId, cache_key: cacheKey });
                return cached;
    ...
    async deleteConnection(connectionId) {
        ...
        const cache = (0, cache_1.getCache)(this.sdkConfig);
        if (cache && this.sdkConfig?.cacheEnabled) {
            const cacheKey = (0, cache_1.generateCacheKey)('DELETE', '/api/v1/brokers/connections/{connection_id}', { connectionId }, this.sdkConfig);
            const cached = cache.get(cacheKey);
            if (cached) {
                this.logger.debug('Cache hit', { request_id: requestId, cache_key: cacheKey });
                return cached;
    ...
    async disconnectBroker(connectionId) {
        ...
        const cache = (0, cache_1.getCache)(this.sdkConfig);
        if (cache && this.sdkConfig?.cacheEnabled) {
            const cacheKey = (0, cache_1.generateCacheKey)('DELETE', '/api/v1/brokers/disconnect/{connection_id}', { connectionId }, this.sdkConfig);
            const cached = cache.get(cacheKey);
            if (cached) {
                this.logger.debug('Cache hit', { request_id: requestId, cache_key: cacheKey });
                return cached;
    ...
    async disconnectCompanyFromBroker(connectionId) {
        ...
        const cache = (0, cache_1.getCache)(this.sdkConfig);
        if (cache && this.sdkConfig?.cacheEnabled) {
            const cacheKey = (0, cache_1.generateCacheKey)('DELETE', '/api/v1/brokers/disconnect-company/{connection_id}', { connectionId }, this.sdkConfig);
            const cached = cache.get(cacheKey);
            if (cached) {
                this.logger.debug('Cache hit', { request_id: requestId, cache_key: cacheKey });
                return cached;
     ...
     */
    async getOrders(brokerId, connectionId, accountId, symbol, orderStatus, side, assetType, limit, offset, createdAfter, createdBefore, withMetadata) {
        // Authentication check
        if (!this.sessionId) {
            throw new Error('Session not initialized. Call startSession() first.');
        }
        ...
        const cache = (0, cache_1.getCache)(this.sdkConfig);
        if (cache && this.sdkConfig?.cacheEnabled) {
            const cacheKey = (0, cache_1.generateCacheKey)('GET', '/api/v1/brokers/data/orders', { brokerId, connectionId, accountId, symbol, orderStatus, side, assetType, limit, offset, createdAfter, createdBefore, withMetadata }, this.sdkConfig);
            const cached = cache.get(cacheKey);
            if (cached) {
                this.logger.debug('Cache hit', { request_id: requestId, cache_key: cacheKey });
                return cached;
     ...
     */
    async getPositions(brokerId, connectionId, accountId, symbol, side, assetType, positionStatus, limit, offset, updatedAfter, updatedBefore, withMetadata) {
        // Authentication check
        if (!this.sessionId) {
            throw new Error('Session not initialized. Call startSession() first.');
        }
        ...
        const cache = (0, cache_1.getCache)(this.sdkConfig);
        if (cache && this.sdkConfig?.cacheEnabled) {
            const cacheKey = (0, cache_1.generateCacheKey)('GET', '/api/v1/brokers/data/positions', { brokerId, connectionId, accountId, symbol, side, assetType, positionStatus, limit, offset, updatedAfter, updatedBefore, withMetadata }, this.sdkConfig);
            const cached = cache.get(cacheKey);
            if (cached) {
                this.logger.debug('Cache hit', { request_id: requestId, cache_key: cacheKey });
                return cached;
     ...
     */
    async getBalances(brokerId, connectionId, accountId, isEndOfDaySnapshot, limit, offset, balanceCreatedAfter, balanceCreatedBefore, withMetadata) {
        // Authentication check
        if (!this.sessionId) {
            throw new Error('Session not initialized. Call startSession() first.');
        }
        ...
        const cache = (0, cache_1.getCache)(this.sdkConfig);
        if (cache && this.sdkConfig?.cacheEnabled) {
            const cacheKey = (0, cache_1.generateCacheKey)('GET', '/api/v1/brokers/data/balances', { brokerId, connectionId, accountId, isEndOfDaySnapshot, limit, offset, balanceCreatedAfter, balanceCreatedBefore, withMetadata }, this.sdkConfig);
            const cached = cache.get(cacheKey);
            if (cached) {
                this.logger.debug('Cache hit', { request_id: requestId, cache_key: cacheKey });
                return cached;
     ...
     */
    async getAccounts(brokerId, connectionId, accountType, status, currency, limit, offset, withMetadata) {
        // Authentication check
        if (!this.sessionId) {
            throw new Error('Session not initialized. Call startSession() first.');
        }
        ...
        const cache = (0, cache_1.getCache)(this.sdkConfig);
        if (cache && this.sdkConfig?.cacheEnabled) {
            const cacheKey = (0, cache_1.generateCacheKey)('GET', '/api/v1/brokers/data/accounts', { brokerId, connectionId, accountType, status, currency, limit, offset, withMetadata }, this.sdkConfig);
            const cached = cache.get(cacheKey);
            if (cached) {
                this.logger.debug('Cache hit', { request_id: requestId, cache_key: cacheKey });
                return cached;
     ...
     */
    async getOrderFills(orderId, connectionId, limit, offset) {
        // Authentication check
        if (!this.sessionId) {
            throw new Error('Session not initialized. Call startSession() first.');
        }
        ...
        const cache = (0, cache_1.getCache)(this.sdkConfig);
        if (cache && this.sdkConfig?.cacheEnabled) {
            const cacheKey = (0, cache_1.generateCacheKey)('GET', '/api/v1/brokers/data/orders/{order_id}/fills', { orderId, connectionId, limit, offset }, this.sdkConfig);
            const cached = cache.get(cacheKey);
            if (cached) {
                this.logger.debug('Cache hit', { request_id: requestId, cache_key: cacheKey });
                return cached;
     ...
     */
    async getOrderEvents(orderId, connectionId, limit, offset) {
        // Authentication check
        if (!this.sessionId) {
            throw new Error('Session not initialized. Call startSession() first.');
        }
        ...
        const cache = (0, cache_1.getCache)(this.sdkConfig);
        if (cache && this.sdkConfig?.cacheEnabled) {
            const cacheKey = (0, cache_1.generateCacheKey)('GET', '/api/v1/brokers/data/orders/{order_id}/events', { orderId, connectionId, limit, offset }, this.sdkConfig);
            const cached = cache.get(cacheKey);
            if (cached) {
                this.logger.debug('Cache hit', { request_id: requestId, cache_key: cacheKey });
                return cached;
     ...
     */
    async getOrderGroups(brokerId, connectionId, limit, offset, createdAfter, createdBefore) {
        // Authentication check
        if (!this.sessionId) {
            throw new Error('Session not initialized. Call startSession() first.');
        }
        ...
        const cache = (0, cache_1.getCache)(this.sdkConfig);
        if (cache && this.sdkConfig?.cacheEnabled) {
            const cacheKey = (0, cache_1.generateCacheKey)('GET', '/api/v1/brokers/data/orders/groups', { brokerId, connectionId, limit, offset, createdAfter, createdBefore }, this.sdkConfig);
            const cached = cache.get(cacheKey);
            if (cached) {
                this.logger.debug('Cache hit', { request_id: requestId, cache_key: cacheKey });
                return cached;
    ...
    async getPositionLots(brokerId, connectionId, accountId, symbol, positionId, limit, offset) {
        ...
        const cache = (0, cache_1.getCache)(this.sdkConfig);
        if (cache && this.sdkConfig?.cacheEnabled) {
            const cacheKey = (0, cache_1.generateCacheKey)('GET', '/api/v1/brokers/data/positions/lots', { brokerId, connectionId, accountId, symbol, positionId, limit, offset }, this.sdkConfig);
            const cached = cache.get(cacheKey);
            if (cached) {
                this.logger.debug('Cache hit', { request_id: requestId, cache_key: cacheKey });
                return cached;
    ...
    async getPositionLotFills(lotId, connectionId, limit, offset) {
        ...
        const cache = (0, cache_1.getCache)(this.sdkConfig);
        if (cache && this.sdkConfig?.cacheEnabled) {
            const cacheKey = (0, cache_1.generateCacheKey)('GET', '/api/v1/brokers/data/positions/lots/{lot_id}/fills', { lotId, connectionId, limit, offset }, this.sdkConfig);
            const cached = cache.get(cacheKey);
            if (cached) {
                this.logger.debug('Cache hit', { request_id: requestId, cache_key: cacheKey });
                return cached;
     ...
     */
    async sandboxCallback(brokerId) {
        // Generate request ID
        const requestId = this._generateRequestId();
        // Input validation (Phase 2B: zod)
        if (this.sdkConfig?.validationEnabled) {
        ...
        const cache = (0, cache_1.getCache)(this.sdkConfig);
        if (cache && this.sdkConfig?.cacheEnabled) {
            const cacheKey = (0, cache_1.generateCacheKey)('GET', '/api/v1/brokers/sandbox-callback/{broker_id}', { brokerId }, this.sdkConfig);
            const cached = cache.get(cacheKey);
            if (cached) {
                this.logger.debug('Cache hit', { request_id: requestId, cache_key: cacheKey });
                return cached;
    ...
    async oauthCallbackTastytrade() {
        ...
        const cache = (0, cache_1.getCache)(this.sdkConfig);
        if (cache && this.sdkConfig?.cacheEnabled) {
            const cacheKey = (0, cache_1.generateCacheKey)('GET', '/api/v1/brokers/callback/tastytrade', {}, this.sdkConfig);
            const cached = cache.get(cacheKey);
            if (cached) {
                this.logger.debug('Cache hit', { request_id: requestId, cache_key: cacheKey });
                return cached;
    ...
    async oauthCallback(brokerId) {
        ...
        const cache = (0, cache_1.getCache)(this.sdkConfig);
        if (cache && this.sdkConfig?.cacheEnabled) {
            const cacheKey = (0, cache_1.generateCacheKey)('GET', '/api/v1/brokers/callback/{broker_id}', { brokerId }, this.sdkConfig);
            const cached = cache.get(cacheKey);
            if (cached) {
                this.logger.debug('Cache hit', { request_id: requestId, cache_key: cacheKey });
                return cached;
     ...
     */
    async placeOrder(body, connectionId) {
        // Authentication check
        if (!this.sessionId) {
            throw new Error('Session not initialized. Call startSession() first.');
        }
        ...
        const cache = (0, cache_1.getCache)(this.sdkConfig);
        if (cache && this.sdkConfig?.cacheEnabled) {
            const cacheKey = (0, cache_1.generateCacheKey)('POST', '/api/v1/brokers/orders', { body, connectionId }, this.sdkConfig);
            const cached = cache.get(cacheKey);
            if (cached) {
                this.logger.debug('Cache hit', { request_id: requestId, cache_key: cacheKey });
                return cached;
     ...
     */
    async cancelOrder(orderId, body, accountNumber, connectionId) {
        // Authentication check
        if (!this.sessionId) {
            throw new Error('Session not initialized. Call startSession() first.');
        }
        ...
        const cache = (0, cache_1.getCache)(this.sdkConfig);
        if (cache && this.sdkConfig?.cacheEnabled) {
            const cacheKey = (0, cache_1.generateCacheKey)('DELETE', '/api/v1/brokers/orders/{order_id}', { orderId, body, accountNumber, connectionId }, this.sdkConfig);
            const cached = cache.get(cacheKey);
            if (cached) {
                this.logger.debug('Cache hit', { request_id: requestId, cache_key: cacheKey });
                return cached;
     ...
     */
    async modifyOrder(orderId, body, accountNumber, connectionId) {
        // Authentication check
        if (!this.sessionId) {
            throw new Error('Session not initialized. Call startSession() first.');
        }
        ...
        const cache = (0, cache_1.getCache)(this.sdkConfig);
        if (cache && this.sdkConfig?.cacheEnabled) {
            const cacheKey = (0, cache_1.generateCacheKey)('PATCH', '/api/v1/brokers/orders/{order_id}', { orderId, body, accountNumber, connectionId }, this.sdkConfig);
            const cached = cache.get(cacheKey);
            if (cached) {
                this.logger.debug('Cache hit', { request_id: requestId, cache_key: cacheKey });
                return cached;
