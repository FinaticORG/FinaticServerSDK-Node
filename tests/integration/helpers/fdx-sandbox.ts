...
export async function createSandboxPortalSession(
  ...
  if ('success' in sessionResponse && !sessionResponse.success) {
    throw new Error(sessionResponse.error ?? 'Failed to start sandbox session');
  }
  const sessionId = v1.getSessionId();
  const companyId = v1.getCompanyId();
  if (!sessionId || !companyId) {
    throw new Error('Sandbox session response is missing session or company context');
  }
